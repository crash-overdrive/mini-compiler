...
object WLP4Gen {
    ...
    def generate_code_function_prolog(global_symbol_table: ListBuffer[FunctionInfo], procedure_name: String) : Unit = {
        ...
        if (procedure_name == "wain") {
            ...
            println("add $29, $25, $0  ; point $29 to temp register $25 which contains start of frame!!")
            println("add $30, $29, $4  ; set stack pointer to bottom of stack!!")
        }
    ...
    def generate_code_push_reg_3() : Unit = {
    ...
    def generate_code_factor(factor_node: Tree, global_symbol_table: ListBuffer[FunctionInfo], procedure_name: String) : Unit = {
        ...
        factor_rule match {
            ...
            case "factor ID LPAREN RPAREN" =>
                var called_function : String = "F" + get_id_from_factor(factor_node)
            ...
            case "factor ID LPAREN arglist RPAREN" =>
                var arglist_child : Tree = factor_node.children(2)
                var called_function : String = "F" + get_id_from_factor(factor_node)
                println(";;call to another function with paramters made!! Save $29, $31 on stack and store stack address on $25!!")
                println("sw $29, -4($30)")
                println("sw $31, -8($30)")
                println("sub $30, $30, $8")
                println("sub $25, $30, $4")
                generate_code_arglist(arglist_child, global_symbol_table, procedure_name)
                println("lis $5")
                println(".word " + called_function)
                println("jalr $5")
                println(";;returned from callee function, $3 <- return value of called function!!")
    ...
    def generate_code_arglist(arglist_node: Tree, global_symbol_table: ListBuffer[FunctionInfo], procedure_name: String) : Unit = {
        var arglist_rule : String = arglist_node.rule 
        arglist_rule match {
            case "arglist expr COMMA arglist" =>
                var expr_child: Tree = arglist_node.children(0)
                println(";;evaluating argument to be pushed on stack!! $3 <- value of expr to be stored on stack!!")
                generate_code_expr(expr_child, global_symbol_table, procedure_name)
                println(";;$3 updated, now pushing it on stack")
                generate_code_push_reg_3()
                generate_code_arglist(arglist_node.children(2), global_symbol_table, procedure_name)
    ...
    def generate_code_procedure(ProcedureS_node: Tree, global_symbol_table: ListBuffer[FunctionInfo]) : Unit = {
        ...
        if (procedure_name != "wain") {
            ...
            generate_code_procedure(procedureS_node.children(1), global_symbol_table)
